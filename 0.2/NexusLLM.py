...
def count_tokens_in_file(file_path):
    try:
        ...
        print(f"Tokens for NexusLMM 0.2: {len(tokens)}")
        return len(tokens)
    except Exception as e:
        print(f"Error reading or tokenizing file: {e}")
        return 0
if __name__ == "__main__":
    count_tokens_in_file("NexusLLM/0.2/NexusTraining.txt")
...
with open('NexusLLM/0.2/NexusTraining.txt', 'r', encoding='utf-8') as f:
    text = f.read()
...
class MiniGPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        return logits, loss
    @torch.no_grad()
    def generate(self, idx, max_new_tokens=30, temperature=0.4):
...
if os.path.exists("NexusLLM/0.2/NexusData.pth"):
    model.load_state_dict(torch.load("NexusLLM/0.2/NexusData.pth", map_location=device))
    print("Training already loaded.")
else:
    print("Training model, [DO NOT TOUCH OR CLOSE THE WINDOW]")
    ...
    torch.save(model.state_dict(), "NexusLLM/0.2/NexusData.pth")
print("\nNexusLLM 0.2: An experimental Large Language Model.")
print("Build: [Unstable]")
print("Lines of Training: [5000]")
print("")
history = ""
while True:
    try:
